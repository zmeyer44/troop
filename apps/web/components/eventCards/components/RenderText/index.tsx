...
const tickerRegex = /^\$[A-Za-z]{3,4}/g;
...
const RenderText = ({ text }: { text?: string }) => {
  ...
  const combinedRegex = new RegExp(
    `(${urlRegex.source}|${hashtagRegex.source}|${nostrPrefixRegex.source}|${tickerRegex.source})`,
    "g",
  );
  // Get Array of URLs
      ...
      if (currentValue?.match(urlRegex)) {
        ...
        specialElement = (
          <Link href={`/?t=${specialValuesArray[index]?.substring(1)}`}>
            <span className="text-primary break-words hover:underline">
              {specialValuesArray[index]}
            </span>
          </Link>
        );
        // specialElement = <span className="">{specialValuesArray[index]}</span>;
      } else if (specialValuesArray[index]?.match(tickerRegex)) {
        specialElement = (
          <Link href={`/?t=${specialValuesArray[index]?.substring(1)}`}>
            <span className="text-primary break-words hover:underline">
              {specialValuesArray[index]}
